...
mod tests {
    ...
    fn test_calorie_counting() {
        let input = load_file_to_vectors(String::from("src/solution/s01_calorie_counting/example.txt"));
        assert_eq!(calorie_counting(input), 24000);
        let input = load_file_to_vectors(String::from("src/solution/s01_calorie_counting/input.txt"));
